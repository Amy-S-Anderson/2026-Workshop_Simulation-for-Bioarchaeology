...
1. **Risk-free** (lesion-related hazard = 1): skeletal lesions have no association with mortality risk.
2. **Risk-doubled** (lesion-related hazard = 2): skeletal lesions are associated with double the baseline mortality risk.
...
params$gammafrailty_shape <- NULL
params$gammafrailty_scale <- NULL
params$lesion_formation_window <- c(0, 10)
params$lesion_formation_rate <- 0.05
params$annual_exposure <- NULL
params$mortality_regime <- CoaleDemenyWestF15

 # rmr = relative mortality risk (lesion-related hazard)

# Helper to run a scenario (fixed params, varying RMR)
run_scenario <- function(params, rmr, nreps) {
  params$lesion_related_hazard <- rmr
  results <- lapply(1:nreps, function(i) {
    out <- do.call(Simulate_Cemetery, params)
    out$individual_outcomes$rep <- i
    out$annual_census$rep <- i
    out
  })
  list(
    cemetery = bind_rows(lapply(results, `[[`, "individual_outcomes")),
    annual_census = bind_rows(lapply(results, `[[`, "annual_census"))
  )
}
...
sim_data <- bind_rows(
  results_rmr1$cemetery %>% mutate(lesion_related_hazard = 1),
  results_rmr2$cemetery %>% mutate(lesion_related_hazard = 2)
) %>%
  mutate(
    scenario = if_else(lesion_related_hazard == 1, "Risk-free", "Risk-doubled"),
    ...
    relative_risk = as.factor(lesion_related_hazard)
  )

# Survivor data (living cohort at each timestep)
sim_survivors <- bind_rows(
  results_rmr1$annual_census %>% mutate(relative_risk = as.factor(1)),
  results_rmr2$annual_census %>% mutate(relative_risk = as.factor(2))
)
```
...
barplot_sample_age_distributions <- sim_data %>%
  ...
  geom_col(aes(group = relative_risk, fill = relative_risk), position = "dodge") +
  scale_fill_manual(values = plot_colors) +
  scale_y_continuous(labels = scales::percent, limits = c(0, .55), expand = c(0, 0)) +
...
Figure_3 <- plot_grid(barplots, legend, ncol = 1, rel_heights = c(1, 0.1))
Figure_3
```
