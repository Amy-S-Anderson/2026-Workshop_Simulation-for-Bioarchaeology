...
#' Build a minimal population data frame for testing
#' @param n Number of agents
#' @param all_alive If TRUE, no agents are marked dead (default TRUE)
create_test_pop <- function(n = 100, all_alive = TRUE) {
  data.frame(
    agent_id  = 1:n,
...
test_that("lesion_formation_rate must be between 0 and 1", {
  ...
  expect_error(
    form_lesions(pop = pop,
                 lesion_formation_window = c(0,80),
...
test_that("formation_window_opens must be non-negative", {
  ...
  expect_error(
    form_lesions(pop,
                 lesion_formation_window = c(-1,10),
...
test_that("formation_window_closes must be >= formation_window_opens", {
  ...
  expect_error(
    form_lesions(pop,
                 lesion_formation_window = c(10,5),# closes before it opens
                 lesion_formation_rate   = 0.05)
  )
})
...
#   pop <- create_test_pop() 
#   pop <- sample_exposure(pop, annual_exposure = 0.3)
#   pop$exposed_this_step <- c(rep(TRUE, 30), rep(FALSE, 70))
# 
#   expect_no_error(
#     form_lesions(pop,
#                  lesion_formation_window = c(18,80),
#                  lesion_formation_rate   = 0.05)
#   )
# })
# 
